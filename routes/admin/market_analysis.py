...
def setup_routes(templates):
    ...
    async def market_analysis_dashboard(request: Request):
        ...
        if tenant_slug.lower() == "all":
            ...
            # Pass None as tenant_id to get data from all stores
            # The market_analysis_service is already designed to handle this case
            sales_trends = market_analysis_service.get_sales_trends(
                tenant_id=None,  # None means all stores
                start_date=start_date.strftime("%Y-%m-%d"),
                end_date=end_date.strftime("%Y-%m-%d")
            )
            
            # Get market insights for all stores
            insights = market_analysis_service.get_market_insights(tenant_id=None)
            
            # Get category performance for all stores
            category_performance = market_analysis_service.get_category_performance(
                tenant_id=None,
                period="month"
            )
            
            logger.info("Retrieved market analysis data for all stores")
        else:
