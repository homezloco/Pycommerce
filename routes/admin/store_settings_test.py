    ...
    if tenant_slug:
        try:
            ...
            if tenant_obj and hasattr(tenant_obj, 'settings'):
                ...
                logger.info(f"[TEST API] Retrieved settings for tenant {tenant_slug}: {settings}")
            else:
                logger.warning(f"[TEST API] Could not retrieve settings for tenant {tenant_slug}")
        except Exception as e:
...
@router.post("/api/store-settings-test", response_class=JSONResponse)
async def store_settings_test_save(
    request: Request
):
    ...
    try:
        ...
        logger.info(f"[TEST API] Received settings update: {settings}")
    except Exception as e:
        logger.error(f"[TEST API] Error parsing JSON: {str(e)}")
        return JSONResponse(
            status_code=400,
    ...
    try:
        ...
        logger.info(f"[TEST API] Updated settings for tenant {tenant_slug}")
        ...
        logger.error(f"[TEST API] Error saving settings: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={"error": f"Error saving settings: {str(e)}"}
        )

@router.post("/api/store-settings-test/reset", response_class=JSONResponse)
async def store_settings_test_reset(
    request: Request
):
    ...
    try:
        ...
        logger.info(f"[TEST API] Reset settings for tenant {tenant_slug}")
        ...
        logger.error(f"[TEST API] Error resetting settings: {str(e)}")
        return JSONResponse(
            status_code=500,
