...
(function() {
    ...
    function connectQuillToForm(quill, formSelector, textareaSelector) {
        try {
            // Find the form - make this more robust by searching for the closest form if not found by selector
            let form = document.querySelector(formSelector);
            if (!form) {
                console.warn(`⚠️ Form not found with selector: ${formSelector}`);
                ...
                if (textarea) {
                    ...
                    if (form) {
                        console.log(`✅ Found alternative form for ${textareaSelector}`);
                    }
                }

                if (!form) {
                    console.error(`❌ Cannot find any suitable form for editor content`);
                    return false;
                }
            }
            ...
            // Add a more robust submit handler without cloning the form
            // This approach preserves other event handlers that might be attached
            const handleSubmit = function(e) {
                // Don't prevent default here - we want to update the content and let the form submit
                try {
                    if (quill && quill.root) {
                        textarea.value = quill.root.innerHTML;
                        console.log(`✅ Updated ${textareaSelector} with editor content (${quill.getText().length} chars)`);
                    } else {
                        console.error("❌ Quill instance not valid when updating content");
                        e.preventDefault();
                    }
                } catch (err) {
            ...
            // Remove any existing handlers we might have set with the same function name
            const handlerName = `quillEditorSubmitHandler_${textareaSelector.replace(/[^a-zA-Z0-9]/g, '_')}`;

            if (form[handlerName]) {
                form.removeEventListener('submit', form[handlerName]);
            }

            // Store the handler on the form element for future reference
            form[handlerName] = handleSubmit;
            form.addEventListener('submit', handleSubmit);

            console.log(`✅ Connected Quill editor to form (${formSelector}) for ${textareaSelector}`);
            return true;
        } catch (error) {
    ...
    function initializeCompleteEditor(config) {
        ...
        if (!targetElement) {
            ...
            if (cfg.currentRetry < cfg.maxRetries) {
                cfg.currentRetry++;
                setTimeout(() => {
                    console.log(`Retry attempt ${cfg.currentRetry}/${cfg.maxRetries} for ${cfg.targetSelector}`);
                    initializeCompleteEditor(cfg);
                }, cfg.retryDelay);
            } else {
        ...
        if (!checkQuillLoaded()) {
            console.log(`Loading Quill resources for ${cfg.targetSelector}...`);
            loadQuillCss();
            loadQuillJs(() => {
                console.log(`Quill loaded, proceeding with editor initialization for ${cfg.targetSelector}`);
                initializeCompleteEditorInternal(cfg);
            });
        } else {
            console.log(`Quill already loaded, initializing editor for ${cfg.targetSelector}`);
            initializeCompleteEditorInternal(cfg);
        }
    }
